...
export const isBodyPart = (name: string): boolean => {
  ...
  return Array.from(BODY_PART_KEYWORDS).some((keyword) =>
...
const MATERIAL_RULES = [
  ...
  {
    pattern: /logo_rear|logo_rear_DENAIL_text|logo_rear_part|logo_front_part/i,
    material: "cardinal",
  },
  {
    pattern:
...
export const resetBodyKitParts = (
  ...
  processBodyParts(scene, (mesh) => {
    ...
    if (originalGeometry) {
      ...
      mesh.material =
        customMaterial || materialCache.getCarPaintMaterial(carColor);
    }
  });
};
...
export const applyBodyKit = (
  ...
  const activePPFOption = ppfOtherColor !== "none" ? ppfOtherColor : option;

  if (activePPFOption === "none") return;

  const materialCache = MaterialCache.getInstance();
  let modifiedParts = 0;

  // Determine if we're using custom color PPF
  const isUsingCustomColor = ppfOtherColor !== "none";
  ...
  processBodyParts(scene, (mesh) => {
    ...
    if (shouldApplyPPF(mesh.name, activePPFOption, vehicleType)) {
      // Dispose old material if it's not a special material
      if (
        mesh.material instanceof THREE.MeshPhysicalMaterial &&
        !getCustomMaterial(mesh.name)
      ) {
        mesh.material.dispose();
      }

      // Apply appropriate PPF material
      if (isUsingCustomColor) {
        // Use custom color for ppfOtherColor option
        mesh.material = materialCache.getPPFMaterialWithCustomColor(carColor);
      } else {
        // Use car-matching color for main PPF option
  ...
  const colorType = isUsingCustomColor ? "custom color" : "car-matching";
  console.log(
    `Applied ${activePPFOption} PPF (${colorType}) to ${modifiedParts} parts`
  );
};
